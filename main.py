...
db = client["Market"]  # Replace with your database name
collection = db["info"]

@app.route('/')
def display_info():
    # Retrieve contact data from MongoDB
    info_collection = db.Students #variable = database.collectionname
...
def display_items():
    # Retrieve contact data from MongoDB
    prepmat_collection = db.items #database.collectionname
    prepmat_11 = prepmat_collection.find()
    return render_template('item_info.html', prepmat_22=prepmat_11)

@app.route('/3_transactions')
def display_transactions():
    # Retrieve contact data from MongoDB
    courses_collection = db.Transaction #database.collectionname
